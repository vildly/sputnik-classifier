...
# --------------------------------------------------
df_melted = df.melt(id_vars=["response", "reference"], value_vars=metric_cols, var_name="metric", value_name="score")
...
# --------------------------------------------------
sns.violinplot(x="reference", y="semantic_similarity", data=df, inner="quartile", palette="Set2", bw=0.2)
...
sns.heatmap(
    matrix, annot=annot, fmt="", cmap="YlGnBu", cbar=False, xticklabels=["Positive", "Negative"], yticklabels=["Positive", "Negative"]
)
