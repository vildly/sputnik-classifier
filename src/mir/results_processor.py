...
async def process_and_store_results_openrouter(model_keys: list):
    """
    Process all job entries for specified models using the OpenRouter response structure.
    ...
    for job in job_documents:
        ...
        for model_key in model_keys:
            ...
            # Collect all entries matching this model_key
            ...
            for model_result in model_results_list:
                # Extraction for OpenRouter: nested choices -> message -> content
                ...
                # Iterate over the parsed content assuming keys are indices of the articles list
                for key, category in parsed_content.items():
                    ...
                    if index < len(articles):
                        ...
                        all_results.append({"user_input": user_input, "reference": reference, "agent_response": agent_response})
                    else:
...
async def process_and_store_results_openai(model_keys: list):
    """
    Process all job entries for specified models using the OpenAI response structure.
    OpenAI structure:
      { "model-name": "results" }
    """
    jobs_col = get_collection(db="jobs", collection="v1")
    ...
    for job in job_documents:
        ...
        for model_key in model_keys:
            ...
            # Process each batch and merge the results
            for model_result in model_results_list:
                # Extraction for OpenAI: the result is directly keyed by model name.
                # Here, model_result is assumed to be the "results" string.
                content = model_result if isinstance(model_result, str) else model_result.get(model_key, "")
                try:
                    parsed_content = json.loads(strip_markdown_fences(content))
                    print(f"Parsed content for job ID {job_id}, model {model_key}: {parsed_content}")
                except json.JSONDecodeError as e:
                ...
                # Iterate over parsed content. Keys should correspond to indices in the articles list.
                for key, category in parsed_content.items():
                    try:
                        index = int(key)
                    except ValueError:
...
async def main():
    ...
    await asyncio.gather(process_and_store_results_openrouter(body.openrouter_models), process_and_store_results_openai(body.openai_models))
